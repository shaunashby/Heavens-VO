...
ok(ref($object) eq 'VO::VOTABLE', "XML file parse returned VO::VOTABLE object.");
...
ok(ref($resource) eq 'VO::RESOURCE',"VOTable contains a RESOURCE block.");
ok($resource->type eq 'results',"VOTable RESOURCE block has type \"results\".");

my $info = $resource->info;
ok(ref($info) eq 'VO::INFO',"VOTable RESOURCE block contains INFO element.");
...
ok(ref($table) eq 'VO::TABLE',"VOTable RESOURCE block has table element.");
ok($table->nrows == 1,"VOTable TABLE element has 1 row.");

my $fields = $table->fields;
ok(ref($fields->[0]) eq 'VO::FIELD',"VOTable TABLE element has FIELD entry.");
...
my $data = $table->data;
ok(ref($data) eq 'VO::DATA',"VOTable TABLE has DATA block.");

my $tabledata = $data->tabledata;
ok(ref($tabledata) eq 'VO::TABLEDATA',"VOTable DATA block has TABLEDATA element.");
ok(scalar(@{$tabledata->rows}) == 1,"VOTable TABLEDATA element has 1 row.");

my $table_row = $tabledata->rows->[0];
ok(ref($table_row) eq 'VO::TR',"VOTable TABLEDATA element has row element TR.");
ok(scalar(@{$table_row->cells}) == 3,"VOTable TABLEDATA row has 3 cells.");

my $row_cells = $table_row->cells;

map {
    ok(ref($_) eq 'VO::TD' && length("$_") > 5, "VOTable TABLEDATA cell has content $_ .");
} @$row_cells;
